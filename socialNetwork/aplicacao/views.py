...
from aplicacao.permissions import *
...
class ApiRoot(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        return Response({
                         'pessoas': reverse(PessoaList.name, request=request),
    ...
    #                            post=post)
        ...
        for user in as_json['users']:
            ...
            address = Address.objects.create(street=user['address']['street'],
                                             ...
                                             zipcode=user['address']['zipcode'],
                                             geo=geo)
            usuario = User.objects.create_user(username = user['username'], email= user['email'], password='123')
            usuario.save()

            pessoa = Pessoa(id=user['id'],
                               name=user['name'],
                               email=user['email'],
                               user = usuario,
                               address=address)
